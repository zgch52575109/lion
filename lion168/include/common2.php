...
require_once(EK_INC.'/libs/ek.process.php');
	 if($cfg_cronnextrun <= time()){
		 getPlayerdetails();
		//updatememberactive();
		   getproxymoney();
	 }
		   echo '开始更新真人视讯投注额。。。'.date('m/d H-i-s'); 
			getnowlivePlayerdetails();
		   echo '真人视讯投注额更新结束。。。'.date('m/d H-i-s'); 
		   //echo '开始更新上周现金码。。。'.date('m/d H-i-s'); 
		   //WeeklyRegularChips();
		   //echo '上周现金码更新结束。。。'.date('m/d H-i-s'); 
		   //echo '开始更新今日现金码。。。'.date('m/d H-i-s'); 
		   //todayRegularChips();
		   //echo '今日现金码更新结束。。。'.date('m/d H-i-s'); 
