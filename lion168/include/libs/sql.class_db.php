...
class DB_MySQL
{
	...
	function EkSql($pconnect=false,$nconnect=true)
	{
	...
	function Init($pconnect=false)
	{
		...
		$this->dbHost   =  $GLOBALS['cfg_dbhost_db'];
		$this->dbUser   =  $GLOBALS['cfg_dbuser_db'];
		$this->dbPwd    =  $GLOBALS['cfg_dbpwd_db'];
		$this->dbName   =  $GLOBALS['cfg_dbname_db'];
		$this->dbPrefix =  $GLOBALS['cfg_dbprefix_db'];
		$this->result["me"] = 0;
		$this->Open($pconnect);
	}

	//用指定参数初始数据库信息
	function SetSource($host,$username,$pwd,$dbname,$dbprefix="ek_")
	{
		$this->dbHost = $host;
	...
	function Open($pconnect=false)
	{
		...
		if(!$this->linkID)
		{
			//$this->DisplayError("EkVod错误警告：<font color='red'>连接数据库失败，可能数据库密码不对或数据库服务器出错！</font>");
			exit();
		}
	...
	//设置SQL语句，会自动把SQL语句里的ek_替换为$this->dbPrefix(在配置文件中为$cfg_dbprefix)
	...
	function DisplayError($msg)
	{
		...
		$emsg = '';
		$emsg .= "<div><h3>EkVod Error Warning!</h3>\r\n";
		$emsg .= "<div><a href='http://bbs.ekeke.net' target='_blank' style='color:red'>Technical Support: http://bbs.ekeke.net</a></div>";
...
function CheckSql($db_string,$querytype='select')
{
	...
	$old_pos = 0;
	$pos = -1;
	$log_file = EK_INC.'/../data/'.md5($cfg_cookie_encode).'_safe.txt';
	$userIP = GetIP();
	$getUrl = GetCurUrl();
