...
function CheckUserEmail($uid, $msgtitle='邮箱',$ckhas=true)
{
	...
	if($ckhas)
	{
		$sql="Select * From `ek_member` where email='$uid'";
		$row = $dsql->GetOne($sql);
...
function Checkuid($uid, $msgtitle='用户名', $ckhas=true)
{
	...
	if($ckhas)
	{
		$row = $dsql->GetOne("Select * From `ek_member` where username like '$uid' ");
		if(is_array($row)) return $msgtitle."已经存在！";
	}
	return 'ok';
}
...
class MemberLogin
{
	...
	function __construct($kptime = -1)
	{
		...
		else
		{
			...
			$this->fields = $dsql->GetOne("Select * From `ek_member` where uid='{$this->M_ID}' ");
			if(is_array($this->fields))
			{
				...
				{
					$dsql->ExecuteNoneQuery("update `ek_member` set logintime='".time()."',loginip='".GetIP()."' where uid='".$this->fields['mid']."';");
	...
	function CheckUser(&$loginuser,$loginpwd)
	{
		...
		$row = $dsql->GetOne("Select uid,password,logintime,username,phone From `ek_member` where username='$loginuser' ");
		if(is_array($row))
		{
