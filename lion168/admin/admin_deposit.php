...
if($action=="save")
{
	...
	$row=$dsql->GetOne("SELECT * FROM `ek_payment_config` where upid = '0' and id='$bankid'");
	if(!is_array($row)){
		ShowMsg("请选择正确的银行卡！","-1");
		exit();
	}
	...
	if($ctype==2)$shouxufei=0;
	if($shouxufei>$cfg_memeber_incash_maxshouxufei) $shouxufei=$cfg_memeber_incash_maxshouxufei;
	$insql="INSERT INTO `ek_member_incash` (orderid,bankid,uid,type,ctype,bank,cardnum,realname,otherbank,cash,hongli,shouxufei,state,`check`,addtime) VALUES ('$orderid','$bankid','$uid','1',$ctype,'$bank','$cardnum','$sender','$otherbank','$cash','$hongli','$shouxufei','$state','$check','".time()."')";
