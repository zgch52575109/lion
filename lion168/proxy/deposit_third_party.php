...
if($action=='save'){
	...
	if($cash<$cfg_memeber_incash_min_money){
		ShowMsg("小值为$cfg_memeber_incash_min_money元","-1");
		exit();
	}
	if($cash>$cfg_memeber_incash_max_money){
		ShowMsg("值为$cfg_memeber_incash_max_money元","-1");
		exit();
	}
	...
	if ($apier['id'] && file_exists(EK_ROOT.'/include/payment/'.$apier['folder'].'.php')){
		...
		$loops['apied'] = $payment->creater($apier['id'],$orderid,'叱值',$cash);
		unset($payment);
	}
	...
	$insql="INSERT INTO `ek_proxy_incash` (orderid,bankid,uid,type,ctype,bank,cash,state,`check`,addtime) VALUES ('$orderid','$bank_id','$cfg_cl->M_ID','1','2','$bank','$cash','$state','$check','".time()."')";
	if($dsql->ExecuteNoneQuery($insql)){
		if ($loops['apied']['method_ispost']){
			...
			echo '						<input id="payment_payer_apied" name="payment_payer_apied" type="submit" value="" />';
			echo '					</form>';
			echo '<script>payment_payer_apied.submit();</script>转支页妫候。?;
		}else{
...
$t->display('proxy/deposit_third_party.html',"$cacheid");