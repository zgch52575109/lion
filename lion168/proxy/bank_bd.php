...
if($action=='edit'){
	...
	$query = "select * from ek_proxy_bank where id='$id' and uid='$cfg_cl->M_ID' ";
	...
	$t->assign('id',$id);
	$t->display('agent/bankadd.html',"$cacheid");
}elseif($action=='save'){
	/*if($id){
		if(!$cfg_memeber_bank_edit){
			ShowMsg("您没有权限编辑银行卡!","-1");
	...
	if(!$id){
		...
		if($cfg_cl->fields['bankmaxnum']){
			echo $bankmaxnum=$cfg_cl->fields['bankmaxnum'];
		}
		if($bankmaxnum){
			$row = $dsql->GetOne("Select count(*) as dd From `ek_proxy_bank` where uid='$cfg_cl->M_ID'");
			if(is_array($row)){
		...
		$row = $dsql->GetOne("Select id From `ek_proxy_bank` where cardnum='$cardnum'");
		$row_member = $dsql->GetOne("Select id From `ek_member_bank` where cardnum='$cardnum'");
		if(is_array($row) || is_array($row_member)){
			ShowMsg("您要绑定的银行卡已存在！","bank_bd.php");
			exit;
		}

		$insql="INSERT INTO `ek_proxy_bank` (uid,sf,city,bankid,zhihang,cardnum,realname,idnumber,addtime) VALUES ('$cfg_cl->M_ID','$sf','$city','$bankid','$zhihang','$cardnum','$realname','$idnumber','".time()."')";
	}else{
		$insql="update `ek_proxy_bank` set sf='$sf',city='$city',bankid='$bankid',zhihang='$zhihang',cardnum='$cardnum',realname='$realname',idnumber='$idnumber' where id='$id' and uid='$cfg_cl->M_ID'";
	}
	if($dsql->ExecuteNoneQuery($insql)){
		$dsql->ExecuteNoneQuery("INSERT INTO `ek_proxy_history_bank` (`uid`,`bankid`,`realname`,`cardnum`,`dateline`)VALUES ('$cfg_cl->M_ID','$bankid','$realname','$cardnum','".time()."')");
	...
	$dsql->ExecuteNoneQuery("delete from ek_proxy_bank where id='$id' and uid='$cfg_cl->M_ID'");
...
$t->assign('bankmaxnum',$cfg_cl->fields['bankmaxnum']);
...
$t->display('agent/bank_bd.html',"$cacheid");